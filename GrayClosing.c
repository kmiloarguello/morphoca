...
graphe * Dilation(struct graphe *g1) {
  ...
  for (i = 0; i < nsom; i++) {
    ...
    for (p = g1->gamma[i]; p != NULL; p = p->next) {
        j = p->som;
        v = p->v_arc;
        // If the successor is greater -> It updates the current vertex with the Max value of successor
        if (g1->v_sommets[j] > g1->v_sommets[i] && g1->v_sommets[i] != INSET ) {
            g->v_sommets[i] = g1->v_sommets[j];
...
graphe * Erosion(struct graphe *g1) {
  ...
  for (i = 0; i < nsom; i++) {
    for (p = g1->gamma[i]; p != NULL; p = p->next) {
        ...
        // If the successor is greater -> It updates the current vertex with the Max value of successor
        if (g1->v_sommets[j] < g1->v_sommets[i] && g1->v_sommets[i] != OUTSET ) {
            g->v_sommets[i] = g1->v_sommets[j];
            AjouteArcValue(g, i, j, (TYP_VARC)v);
        }
    }
