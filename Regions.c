...
struct xvimage * FindNeighborhs(struct xvimage *image, int32_t distance) {
    ...
    for (j = 0; j < cs; j++) {
        for (i = 0; i < rs; i++)  
        {
            ...
            if (i < (rs-1)) {
                ...
                if( abs(F[j*rs+i] - F[j*rs+i+1]) <= distance ) {
                    F[j*rs+i+1] = F[j*rs+i];
                }
            ...
            if (j < (cs-1)) {
                ...
                if( abs(F[j*rs+i] - F[(j+1)*rs+i]) <= distance ) {
                    F[(j+1)*rs+i] = F[j*rs+i]; 
                }
            }
...
graphe * SetRegions(struct graphe *g1) {
  ...
  for (i = 0; i < nsom; i++) {
    ...
    for (p = g1->gamma[i]; p != NULL; p = p->next) {
        ...
        printf("%d \n", abs(g1->v_sommets[i] - g1->v_sommets[j]));

        if ( abs(g1->v_sommets[i] - g1->v_sommets[j]) < 50 ) {
            g->v_sommets[i] = g1->v_sommets[i];
...
int main(int argc, char ** argv){
  ...
  if (argc != 3)
  {
    fprintf(stderr, "usage: %s im.pgm out.pgm \n", argv[0]);
    exit(1);
  }
  ...
  printf("Region Adjacency started! \n");
  image = FindNeighborhs(image, 40);
